...
export class StarboardClient {
    ...
    private generateEdit(starCount: number, message: Message): MessageEditOptions {
        return {
            ...
            embeds: [
                new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "Kaynak", value: `[Atla](${message.url})` }
                    )
    ...
    private generateCreate(starCount: number, message: Message): MessageCreateOptions {
        return {
            content: `${starCount} ⭐ | ${message.channel}`,
            embeds: [
                new EmbedBuilder()
    ...
    public async listener(reaction: MessageReaction) {
        ...
        const generateCreate = this.generateCreate(reaction.count, reaction.message as Message)
        const generateEdit = this.generateEdit(reaction.count, reaction.message as Message)
        
        const sendMessage = () => {
            starboardChannel?.send(generateCreate).then((m) => {
