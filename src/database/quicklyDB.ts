...
interface quicklyDBEvents {
    ready: (quicklyDB: quicklyDB) => unknown
}

export class quicklyDB {
    public schema = mongoose.model(
        "quicklydb-collection",
        new Schema({
            key: String,
    ...
    constructor(mongooseConnectionString: string) {
        if (mongoose.connection.readyState !== 1) {
            if (!mongooseConnectionString)
                throw new Error(
                    "Mongoose ile herhangi bir kurulu bağlantı yok ve bir Mongoose bağlantısı gerekiyor!"
                )
     ...
     * @example <quicklyDB>.set("test","js is cool")
     */
    ...
    public push(key: string, ...pushValue: any) {
        ...
        if (!Array.isArray(data))
            throw Error(`${typeof data} türündeki bir değere veri gönderemezsiniz.!`)
...
export interface quicklyDBSchema {
    key: string
    value: any
}